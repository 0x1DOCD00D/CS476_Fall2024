...
## Overview
In your homework assignments you created a simple digital fuzzy expression evaluation language embedded in Scala that uses a dynamically updated environment to specify values for variables with computing and storing results of the evaluation of arithmetic expressions in variables and using them in different scopes as well as dynamic dispatch algorithms for resolving method invocations at runtime. In this third homework you will work on adding an optimization for partial evaluation, a programming language technique that allow undefined variables to remain in expressions during evaluation while preserving the syntactical form of the expressions. Consider the following example of partial evaluation.
```scala
